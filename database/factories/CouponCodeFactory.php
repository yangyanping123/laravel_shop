...
use App\Http\Models\Enum\CouponCodeEnum;
$factory->define(CouponCode::class, function (Faker $faker) {
    // 首先随机取得一个类型
    $type  = $faker->randomElement(array_keys(CouponCodeEnum::$typeMap));
    // 根据取得的类型生成对应折扣
    $value = $type === CouponCodeEnum::TYPE_FIXED ? random_int(1, 200) : random_int(1, 50);

    // 如果是固定金额，则最低订单金额必须要比优惠金额高 0.01 元
    if ($type === CouponCodeEnum::TYPE_FIXED) {
