    ...
    <div class="box-body">
        <table class="table table-bordered">
            ...
            <tr>
                ...
                <!-- 这里也新增了一个发货状态 -->
                <td>发货状态：</td>
                <td>{{ \App\Http\Models\Enum\OrderEnum::getShipStatusName($order->ship_status) }}</td>
            </tr>
            <!-- 订单发货开始 -->
            <!-- 如果订单未发货，展示发货表单 -->
           ...
           ($order->type !== \App\Http\Models\Enum\OrderEnum::TYPE_CROWDFUNDING ||
            ...
            @endif
            @if($order->refund_status !== \App\Http\Models\Enum\OrderEnum::REFUND_STATUS_PENDING)
                <tr>
                    <td>退款状态：</td>
                    <td colspan="2">{{ \App\Http\Models\Enum\OrderEnum::getRefundStatusName($order->refund_status) }}，理由：{{ $order->extra['refund_reason'] }}</td>
                    <td>
                        <!-- 如果订单退款状态是已申请，则展示处理按钮 -->
                        @if($order->refund_status === \App\Http\Models\Enum\OrderEnum::REFUND_STATUS_APPLIED)
