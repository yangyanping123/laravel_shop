              ...
              <span class="product-title">
                 <a target="_blank" href="{{ route('products.show', [$item->product_id]) }}">{{ $item->product->title }}</a>
                    ...
                    <div class="order-bottom">
                        ...
                        <!-- 订单已支付，且退款状态不是未退款时展示退款信息 -->
                            @if($order->paid_at && $order->refund_status !== \App\Http\Models\Enum\OrderEnum::REFUND_STATUS_PENDING)
                                <div class="line">
                                    ...
                                    <div class="line-value">{{ \App\Http\Models\Enum\OrderEnum::getRefundStatusName($order->refund_status) }}</div>
                                </div>
                                <div class="line">
                                ...
                                <div class="value">
                                    @if($order->paid_at)
                                        @if($order->refund_status === \App\Http\Models\Enum\OrderEnum::REFUND_STATUS_PENDING)
                                            已支付
                                        @else
                                            {{ \App\Http\Models\Enum\OrderEnum::getRefundStatusName($order->refund_status)}}
                                        @endif
                                    @elseif($order->closed)
                                        已关闭
                                    @else
                        ...
                        @if($order->ship_status === App\Http\Models\Enum\OrderEnum::SHIP_STATUS_DELIVERED)
