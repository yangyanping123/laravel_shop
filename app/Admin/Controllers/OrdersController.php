...
class OrdersController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->ship_status('物流')->display(function($value) {
            return OrderEnum::getShipStatusName($value);
        });
        $grid->refund_status('退款状态')->display(function($value) {
            return OrderEnum::getRefundStatusName($value);
        });
        // 禁用创建按钮，后台不需要创建订单
    ...
    public function showdata(Order $order, Content $content)
    {
        return $content
            ->header('查看订单')
    ...
    public function ship(Order $order,OrderDelivateRequest $request)
    {
      //  dd($request);
        // 判断当前订单是否已支付
        if (!$order->paid_at) {
        ...
        $order->update([
            'ship_status' => OrderEnum::SHIP_STATUS_DELIVERED,
            // 我们在 Order 模型的 $casts 属性里指明了 ship_data 是一个数组
            // 因此这里可以直接把数组传过去
            'ship_data'   => $request->only([
                'express_company',
                'express_no'
            ]),
        ]);
    ...
    public function handleRefund(Order $order, HandleRefundRequest $request)
    {
        ...
        if ($request->input('agree')) {
            ...
            // 将订单的退款状态改为未退款
            $order->update([
                'refund_status' => OrderEnum::REFUND_STATUS_PENDING,
                'extra'         => $extra,
            ]);
