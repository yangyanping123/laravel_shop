...
class InstallmentsController extends Controller
{
    ...
    public function payByAlipay(Installment $installment)
    {
        ...
        if ($installment->status === InstallmentEnum::STATUS_FINISHED) {
    ...
    public function alipayNotify()
    {
        ...
        \DB::transaction(function () use ($data, $no, $installment, $item) {
            ...
            if ($item->sequence === 0) {
                // 将分期付款的状态改为还款中
                $installment->update(['status' => InstallmentEnum::STATUS_REPAYING]);
                // 将分期付款对应的商品订单状态改为已支付
            ...
            if ($item->sequence === $installment->count - 1) {
                // 将分期付款状态改为已结清
                $installment->update(['status' => InstallmentEnum::STATUS_FINISHED]);
            }
        });
